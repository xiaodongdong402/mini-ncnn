/*************************************************************************
	> File Name: low_precision_convolution.cpp
	> Author: dzhang
	> Mail: dong.zhang@ingenic.com 
	> Created Time: Fri 12 Apr 2019 04:48:34 PM CST
 ************************************************************************/
...
//#ifndef REGISTER_LPCONVOLUTION
//DEFINE_LAYER_CREATOR(LPConvolution)
//#define REGISTER_LPCONVOLUTION
//#endif

//Layer* LPConvolution_layer_creator() { return new LPConvolution; }

DEFINE_LAYER_CREATORD(LPConvolution)

LPConvolution::LPConvolution()
{
    one_blob_only = true;
...
int LPConvolution::load_param(const ParamDict& pd)
{
...
int LPConvolution::load_model(const ModelBin& mb)
{
...
int LPConvolution::forward(const Mat& bottom_blob, Mat& top_blob, const Option& opt) const
{
    // convolv with NxN kernel
    // value = value + bias

	LOGCC << "LPConvolution forward!!!" <<endl;
...
//     fprintf(stderr, "LPConvolution input %d x %d  pad = %d %d  ksize=%d %d  stride=%d %d\n", w, h, pad_w, pad_h, kernel_w, kernel_h, stride_w, stride_h);
...
#if NCNN_VULKAN
int LPConvolution::upload_model(VkTransfer& cmd)
{
    const int maxk = kernel_w * kernel_h;
...
int LPConvolution::create_pipeline()
{
...
int LPConvolution::destroy_pipeline()
{
    if (padding)
...
int LPConvolution::forward(const VkMat& bottom_blob, VkMat& top_blob, VkCompute& cmd, const Option& opt) const
{
    int w = bottom_blob.w;
    int h = bottom_blob.h;
    int channels = bottom_blob.c;
